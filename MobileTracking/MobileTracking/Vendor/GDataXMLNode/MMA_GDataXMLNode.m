...
// MMA_GDataXMLNode does not require pulling in all of GData.
...
static NSString *GDataFakeQNameForURIAndName(NSString *theURI, NSString *name) {
  ...
  NSString *localName = [MMA_GDataXMLNode localNameForName:name];
...
@interface MMA_GDataXMLNode (PrivateMethods)
...
- (MMA_GDataXMLNode *)attributeForXMLNode:(xmlAttrPtr)theXMLNode;
...
@implementation MMA_GDataXMLNode
...
#if DEBUG
  NSCAssert(chars != NULL, @"MMA_GDataXMLNode sees an unexpected empty string");
#endif
...
- (NSArray *)children {
  ...
  if (xmlNode_ != NULL) {
    ...
    while (currChild != NULL) {
      MMA_GDataXMLNode *node = [MMA_GDataXMLNode nodeBorrowingXMLNode:currChild];
...
- (MMA_GDataXMLNode *)childAtIndex:(unsigned)index {
    ...
    if (xpathCtx) {
      ...
      if (namespaces) {
        ...
        for (NSString *prefix in namespaces) {
          ...
          if (result != 0) {
#if DEBUG
            NSCAssert1(result == 0, @"MMA_GDataXMLNode XPath namespace %@ issue",
                      prefix);
#endif
          }
        }
        ...
        if (nsNodePtr != NULL) {
          for (xmlNsPtr nsPtr = nsNodePtr->ns; nsPtr != NULL; nsPtr = nsPtr->next) {
            ...
            if (prefix == NULL) {
              prefix = (xmlChar*) kMMAGDataXMLXPathDefaultNamespacePrefix;
            }

            int result = xmlXPathRegisterNs(xpathCtx, prefix, nsPtr->href);
            if (result != 0) {
#if DEBUG
              NSCAssert1(result == 0, @"MMA_GDataXMLNode XPath namespace %s issue",
                        prefix);
#endif
            }
          }
      ...
      if (xpathObj) {
        ...
        if (nodeSet) {
          ...
          for (int index = 0; index < nodeSet->nodeNr; index++) {
            ...
            MMA_GDataXMLNode *node = [MMA_GDataXMLNode nodeBorrowingXMLNode:currNode];
            if (node) {
              [array addObject:node];
            }
          }
...
- (BOOL)isEqual:(MMA_GDataXMLNode *)other {
  if (self == other) return YES;
  if (![other isKindOfClass:[MMA_GDataXMLNode class]]) return NO;
...
- (NSUInteger)hash {
  return (NSUInteger) (void *) [MMA_GDataXMLNode class];
}
  ...
  if (xmlNode_ != NULL && xmlNode_->nsDef != NULL) {
    ...
    while (currNS != NULL) {
      ...
      if (!xmlStrEqual(currNS->prefix, (const xmlChar *) "xml")) {
        MMA_GDataXMLNode *node = [MMA_GDataXMLNode nodeBorrowingXMLNode:(xmlNodePtr) currNS];
...
- (void)setNamespaces:(NSArray *)namespaces {
  ...
  if (xmlNode_ != NULL) {
    ...
    MMA_GDataXMLNode *namespaceNode;
...
- (void)addNamespace:(MMA_GDataXMLNode *)aNamespace {
...
- (void)removeChild:(MMA_GDataXMLNode *)child {
...
- (NSArray *)elementsForName:(NSString *)name {
  ...
  if (xmlNode_ != NULL) {
    ...
    for (MMA_GDataXMLNode *child in children) {
...
- (NSArray *)attributes {
  ...
  if (xmlNode_ != NULL && xmlNode_->properties != NULL) {
    ...
    while (prop != NULL) {
      ...
      MMA_GDataXMLNode *node = [MMA_GDataXMLNode nodeBorrowingXMLNode:(xmlNodePtr) prop];
      if (array == nil) {
        array = [NSMutableArray arrayWithObject:node];
      } else {
        [array addObject:node];
...
- (void)addAttribute:(MMA_GDataXMLNode *)attribute {
...
- (MMA_GDataXMLNode *)attributeForXMLNode:(xmlAttrPtr)theXMLNode {
  // search the cached attributes list for the MMA_GDataXMLNode with
  // the underlying xmlAttrPtr
  NSArray *attributes = [self attributes];

  for (MMA_GDataXMLNode *attr in attributes) {
...
- (MMA_GDataXMLNode *)attributeForName:(NSString *)name {
  ...
  if (xmlNode_ != NULL) {
    ...
    if (attrPtr) {
      MMA_GDataXMLNode *attr = [self attributeForXMLNode:attrPtr];
      return attr;
    }
  }
  return nil;
...
- (MMA_GDataXMLNode *)attributeForLocalName:(NSString *)localName
